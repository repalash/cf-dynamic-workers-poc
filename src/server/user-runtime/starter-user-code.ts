// @ts-nocheck
// The default user.js shipped into the dynamic worker. Editable via the
// admin UI. Exports a Hono app — the worker entry imports it and mounts it
// at / (pattern matches packages/notes-sample/hono-jsx-ssr/app.tsx).
//
// Plain JS — LOADER.load won't transpile JSX/TS. Uses html`…` tagged
// templates from hono/html (re-exported from the teenybase bundle) instead
// of JSX.
...
const styles = `
  *{box-sizing:border-box}
  ...
  input:focus,textarea:focus{outline:none;border-color:#ff8c42;box-shadow:0 0 0 1px rgba(255,140,66,0.35)}
...
app.get("/notes/:id", async (c) => {
  ...
  try {
    ...
    return c.html(
      layout("Error", html`<h1>Error</h1><div class="err">${e?.message ?? String(e)}</div>`, user),
      500
    )
  }
})
